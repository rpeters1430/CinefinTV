...
class EncryptedPreferences @Inject constructor(
    ...
    fun encryptValue(value: String): String? {
        ...
        return try {
            ...
            if (iv == null || iv.size != IV_LENGTH) {
                SecureLogger.e(TAG, "Encryption failed - invalid IV generated by Keystore")
                return null
            }
            ...
            if (BuildConfig.DEBUG) {
                SecureLogger.e(TAG, "Encryption failed - security error", e)
            }
    ...
    fun decryptValue(encryptedValue: String?): String? {
        ...
        return try {
            ...
            if (combined.size < IV_LENGTH + 16) { // 16 bytes = GCM authentication tag
                if (BuildConfig.DEBUG) {
                    SecureLogger.e(TAG, "Encrypted data too short - may be corrupted")
                }
                return null
            }
            ...
            if (BuildConfig.DEBUG) {
                SecureLogger.e(TAG, "Decryption failed - data may be corrupted, tampered, or using wrong key", e)
            }
    ...
    suspend fun putEncryptedString(key: String, value: String?) {
        ...
        if (encrypted != null) {
            ...
            SecureLogger.e(TAG, "Failed to encrypt value for key: $key")
        }
    }
    ...
    fun getEncryptedString(key: String): Flow<String?> {
        return context.secureDataStore.data
            .catch { exception ->
                SecureLogger.e(TAG, "Error reading secure preferences", exception)
